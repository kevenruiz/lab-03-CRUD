...
class Order {
  ...
  static async update(quantityOfItems, id) {
    ...
    const { rows } = await pool.query(
      'UPDATE orders WHERE (id) VALUES ($1) RETURNING *',
      [id, quantityOfItems]
    );
    return new Order(rows[0]);
  }

  static async get(quantityOfItems, id) {
  ...
  static async delete(quantityOfItems, id) {
  ...
  static async getByI(quantityOfItems, id) {
